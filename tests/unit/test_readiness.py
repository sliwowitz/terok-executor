...
class TestProtocolRollup:
    ...
    def test_live_custom_provider_augments_stale_baked_universe(
        ...
        env = {
            **AUTHED_ENV,
            "TEROK_PROVIDER_EXAMPLE_TOKEN": "terok-p-custom",
            "TEROK_PROVIDER_EXAMPLE_BASE_OPENAI_CHAT": f"{_LOOPBACK}/v1",
        }

        manifest = generator.build_manifest(set(AGENT_FACTS), AGENT_FACTS, AGENT_UNIVERSE, env)

        chat = self._row(manifest, "openai-chat")
        assert "example" in chat["candidates"]
        assert "example" in chat["authenticated"]
        assert _pair(manifest, "vibe", "example")["ready"] is True
